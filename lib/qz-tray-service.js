        ...
        while (attempts < maxAttempts) {
          try {
            ...
            if (attempts < maxAttempts) {
              console.log(`[v0] Tentativa ${attempts} - Nenhuma impressora encontrada, tentando novamente...`)
              await new Promise((resolve) => setTimeout(resolve, 1000))
            }
          } catch (error) {
            attempts++
            console.error(`[v0] Erro na tentativa ${attempts} de buscar impressoras:`, error)
            if (attempts >= maxAttempts) {
              throw error
            }
          }
