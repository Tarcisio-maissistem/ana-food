    ...
    try {
      ...
      qz.security.setCertificatePromise(() => {
        ...
        const certSources = [
          ...
          "/api/qz/certificate", // Local fallback endpoint
        ]

        const tryFetchCertificate = async (url, index = 0) => {
          try {
            console.log(`[v0] Tentando buscar certificado da fonte ${index + 1}:`, url)
            ...
            if (!cert || cert.length < 100 || !cert.includes("BEGIN CERTIFICATE")) {
              throw new Error("Certificado inválido ou vazio")
            }

            console.log("[v0] Certificado compartilhado carregado com sucesso")
            return cert
          } catch (error) {
            console.warn(`[v0] Falha ao buscar certificado da fonte ${index + 1}:`, error.message)

            // Try next source
            if (index < certSources.length - 1) {
              return tryFetchCertificate(certSources[index + 1], index + 1)
            }
        ...
        return tryFetchCertificate(certSources[0]).catch((error) => {
          console.error("[v0] Erro ao buscar certificado compartilhado:", error.message)
          console.log("[v0] Usando certificado mock para desenvolvimento")

          // Fallback to mock certificate for development
          ...
          fetch("/api/qz/sign", {
            ...
            .then((data) => {
              if (data.signature) {
                ...
                throw new Error("Assinatura não recebida do servidor")
              }
            })
            .catch((error) => {
              console.error("[v0] Erro na assinatura compartilhada:", error)
              // Fallback to mock signature
              const mockSignature = "SHA256withRSA:" + btoa(toSign + "shared-signature-key")
