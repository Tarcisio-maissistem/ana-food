...
class QZTrayService {
  ...
  getQzTrayGuidance() {
    return {
      ...
      instructions: [
        ...
        "4. Certifique-se de que está rodando na porta 8182",
      ...
      troubleshooting: [
        ...
        "• Verifique se não há firewall bloqueando a porta 8182",
    ...
    try {
      if (!this.isConnected) {
        ...
        if (!connectionResult.success && !connectionResult) {
          // If connection failed, provide demo printers with guidance
          console.log("[v0] Conexão falhou, fornecendo impressoras de demonstração")
          this.printers = [
            "🖨️ Demo - Microsoft Print to PDF",
            "🖨️ Demo - Impressora Virtual",
            "🖨️ Demo - Teste de Impressão",
          ]
          return {
            printers: this.printers,
            isDemo: true,
            guidance: connectionResult.guidance || this.getQzTrayGuidance(),
          }
        }
      }
      ...
      console.log("[v0] Todos os métodos falharam, fornecendo impressoras de demonstração...")

      // Provide demo printers when real detection fails
      this.printers = [
        "🖨️ Demo - Microsoft Print to PDF",
        "🖨️ Demo - Microsoft XPS Document Writer",
        "🖨️ Demo - Impressora Virtual",
        "🖨️ Demo - Teste de Impressão",
      ]

      console.log("[v0] Impressoras de demonstração carregadas:", this.printers)
      return {
        printers: this.printers,
        isDemo: true,
        message: "QZ Tray conectado, mas não foi possível detectar impressoras reais. Usando modo demonstração.",
      }
    } catch (error) {
      console.error("[v0] Erro crítico ao listar impressoras:", error.message)

      this.printers = ["🖨️ Demo - Impressora Virtual (Teste)", "🖨️ Demo - Simulação de Impressão"]

      console.log("[v0] Fallback final - modo demonstração:", this.printers)
      return {
        printers: this.printers,
        isDemo: true,
        error: error.message,
  ...
  async printOrder(order) {
    try {
      ...
      if (this.selectedPrinter.startsWith("🖨️ Demo")) {
        console.log("[v0] Impressão em modo demonstração")
        console.log("[v0] Conteúdo que seria impresso:")
        console.log(content)
        ...
        return {
          success: true,
          isDemo: true,
          message: "Impressão simulada com sucesso! Verifique o console para ver o conteúdo.",
          content: content,
        }
      }
