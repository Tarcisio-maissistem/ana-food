...
class QZTrayService {
  ...
  async listPrinters() {
    try {
      ...
      let attempts = 0
      const maxAttempts = 2 // Reduced attempts to avoid excessive retries

      while (attempts < maxAttempts) {
        try {
          attempts++
          console.log(`[v0] Tentativa ${attempts} de buscar impressoras...`)

          const printerPromise = window.qz.printers.find()
          const timeoutPromise = new Promise((_, reject) =>
            setTimeout(() => reject(new Error("Timeout ao buscar impressoras (15s)")), 15000),
          )

          console.log("[v0] Aguardando resposta do QZ Tray...")
          this.printers = await Promise.race([printerPromise, timeoutPromise])
          console.log("[v0] Resposta recebida do QZ Tray:", this.printers)

          if (this.printers && this.printers.length > 0) {
            console.log(`[v0] ${this.printers.length} impressora(s) detectada(s):`, this.printers)
            break
          } else {
            console.log("[v0] QZ Tray retornou lista vazia de impressoras")
          }

          if (attempts < maxAttempts) {
            console.log(
              `[v0] Tentativa ${attempts} - Nenhuma impressora encontrada, aguardando antes da próxima tentativa...`,
            )
            await new Promise((resolve) => setTimeout(resolve, 3000)) // Increased wait time
          }
        } catch (error) {
          console.error(`[v0] Erro na tentativa ${attempts} de buscar impressoras:`, error.message)

          if (attempts >= maxAttempts) {
            console.error("[v0] Falha ao buscar impressoras após todas as tentativas")
            console.log("[v0] Possíveis soluções:")
            console.log("1. Verifique se o QZ Tray está executando (ícone na bandeja do sistema)")
            console.log("2. Execute o QZ Tray como administrador")
            console.log("3. Verifique se há impressoras instaladas no Windows")
            console.log("4. Reinicie o QZ Tray e tente novamente")
            console.log("5. Verifique se o firewall não está bloqueando o QZ Tray")
      ...
      if (!this.printers || this.printers.length === 0) {
        console.warn("[v0] Nenhuma impressora encontrada após todas as tentativas")
        this.printers = []
      }
