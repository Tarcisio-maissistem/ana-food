  ...
  async loadQzTrayScript() {
    return new Promise((resolve, reject) => {
      ...
      script.onload = () => {
        ...
        // Wait for QZ to be fully initialized
  ...
  async setupSecurity() {
    try {
      if (!window.qz || !window.qz.security) {
        throw new Error("QZ Tray security not available")
      }
...
-----END CERTIFICATE-----`)
  ...
  async connect() {
    try {
      ...
      // Wait for QZ Tray to be fully available
      await new Promise((resolve) => setTimeout(resolve, 500))

      if (!window.qz) {
        throw new Error("QZ Tray script não carregado corretamente")
      }
      ...
      if (!window.qz.websocket || !window.qz.websocket.connect) {
        throw new Error("QZ Tray websocket não disponível")
      }
      ...
      const guidance = {
        ...
        instructions: [
          ...
          "4. Certifique-se de que o QZ Tray está rodando na porta 8182",
  ...
  async listPrinters() {
    try {
      ...
      // Verify QZ Tray printers API is available
      if (!window.qz.printers || !window.qz.printers.findAll) {
        throw new Error("QZ Tray printers API não disponível")
      }

      console.log("[v0] Buscando impressoras do Windows...")

      // Use findAll() as shown in the working example
