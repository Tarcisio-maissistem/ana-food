...
class QZTraySilentService {
  constructor() {
    ...
    this.connectionTimeout = 5000 // Reduced timeout for faster connection
  ...
  setupSecurityBypass() {
    try {
      // Desabilitar completamente a validação de certificados
  ...
  async connect() {
    try {
      ...
      // Conectar com timeout reduzido para rapidez
      await Promise.race([
      ...
      return {
        success: false,
        error: error.message,
        suggestion: "Verifique se o QZ Tray está rodando com config.js configurado",
      }
    }
  }
