...
class QZTrayService {
  ...
  async listPrinters() {
    try {
      ...
      if (typeof window === "undefined" || !window.qz) {
        throw new Error("QZ Tray não está disponível - window.qz não encontrado")
      }
      ...
      while (attempts < maxAttempts) {
        try {
          ...
          const timeoutPromise = new Promise((_, reject) =>
            setTimeout(() => reject(new Error("Timeout ao buscar impressoras")), 10000),
          )
          ...
          if (attempts >= maxAttempts) {
            throw new Error(`Falha ao buscar impressoras após ${maxAttempts} tentativas: ${error.message}`)
          }
      ...
      if (!this.printers || this.printers.length === 0) {
        ...
        console.log("[v0] Verifique se:")
        console.log("- QZ Tray está executando como administrador")
        console.log("- Há impressoras instaladas no sistema")
        console.log("- As impressoras estão online e funcionando")
        console.log("- O QZ Tray tem permissões para acessar as impressoras")
