...
class QZTrayService {
  ...
  async listPrinters() {
    try {
      ...
      try {
        // Verify connection is still active before attempting printer search
        const stillConnected = await this.isQzConnected()
        if (!stillConnected) {
          throw new Error("Conexão perdida durante busca")
        }
        ...
        if (error.message.includes("Connection closed") || error.message.includes("Conexão perdida")) {
          console.log("[v0] Tentando reconectar após perda de conexão...")
          await this.connect()
        }
      }
      ...
      // Return fallback printers immediately without trying to use QZ Tray connection
      ...
      this.printers = ["Microsoft Print to PDF", "Impressora Virtual (Teste)", "Fallback Printer"]
  ...
  async isQzConnected() {
    try {
      if (typeof window !== "undefined" && window.qz && window.qz.websocket) {
        ...
        if (isActive) {
          try {
            // Test if we can actually communicate with QZ Tray
            await Promise.race([
              new Promise((resolve) => {
                // Simple test to verify connection is responsive
                if (window.qz.printers) {
                  resolve(true)
                } else {
                  resolve(false)
                }
            ...
            console.log("[v0] Conexão QZ Tray não está responsiva:", testError.message)
            return false
          }
        }
