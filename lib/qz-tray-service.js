...
class QZTraySilentService {
  ...
  async connect() {
    try {
      ...
      try {
        ...
        console.warn("[v0] Erro na configuração de segurança:", securityError.message)
      }
      ...
      for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
        try {
          ...
          if (qz.websocket.isActive()) {
            ...
            throw new Error("Conexão estabelecida mas não ativa")
          }
        } catch (error) {
          lastError = error
  ...
  generatePreviewContent(order, company) {
    // Use the same content generation logic as printing but for preview
...
\x1D\x56\x42\x00`
  ...
  getConnectionSuggestion(error) {
    ...
    if (error?.message?.includes("WebSocket")) {
      return "Problema de comunicação WebSocket. Reinicie o QZ Tray e tente novamente."
    }
    if (error?.message?.includes("not found") || error?.message?.includes("404")) {
      return "QZ Tray não foi encontrado. Verifique se está instalado corretamente."
    }
    return "Verifique se o QZ Tray está instalado e rodando corretamente. Tente reiniciar o aplicativo."
  }
}
